...
export default function Footer() {
    ...
    const linkedinUrl = resumeData.basics.profiles.find(p => p.network === "LinkedIn")?.url || "#";
        ...
        <footer className="border-t py-6 md:py-0">
            <div className="container flex flex-col items-center justify-between gap-4 md:h-24 md:flex-row mx-auto px-4 md:px-6">
                <p className="text-center text-sm leading-loose text-muted-foreground md:text-left">
                    Built by <span className="font-semibold">{resumeData.basics.name}</span>.
                    Based in {resumeData.basics.location.countryCode}.
                </p>
