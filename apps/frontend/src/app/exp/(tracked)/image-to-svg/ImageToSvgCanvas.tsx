...
export interface ImageToSvgCanvasProps {
    ...
    /** Output paper size id (see `paper.ts`). */
    paperId: string;
    /** Artwork zoom; 1 = fit the page. */
    zoom: number;
...
export function ImageToSvgCanvas({
    ...
    useEffect(() => {
        const handle = window.setTimeout(() => {
            ...
            const out = applyLayout(
                getStrategy(strategyId).render(input, params),
                paperId,
                zoom,
            );
            setSvg(out);
    ...
    }, [input, strategyId, params, paperId, zoom, onSvgChange]);
